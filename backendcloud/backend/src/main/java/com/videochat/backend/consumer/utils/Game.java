...
public class Game extends Thread {
    ...
    public Game(Integer rows, Integer cols, Integer idA, Integer idB, Bot botA, Bot botB) {
        this.rows = rows;
        this.cols = cols;
    ...
    private boolean getNextStep() {
        if (nextPlayer == playerA.getId()) sendBotCode(playerA);
        if (nextPlayer == playerB.getId()) sendBotCode(playerB);
    ...
    private boolean check_valid(List<Cell> cur_cells, List<Cell> bef_cells, int id) {
        if (cur_cells.size() > max_num) return false;
        createMap();
        for (Cell cell : bef_cells) {
            g[cell.x][cell.y] = 5 - id;
        }

        for (Cell cell : cur_cells) {
            if (g[cell.x][cell.y] != 0 && g[cell.x][cell.y] != id + 2) return false;
            g[cell.x][cell.y] = id;
        }
    ...
    private boolean judge() {
        ...
        if (nextPlayer == playerA.getId()) {
            if (!playerA.checkCells() || !check_valid(cellsA, cellsB, 2)) return false;
            ...
            if (!playerB.checkCells() || !check_valid(cellsB, cellsA, 3)) return false;
